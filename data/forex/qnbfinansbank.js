...
async function getQNBFinansBankUSD() {
  try {
...
async function getQNBFinansBankEUR() {
  try {
    const response = await axios({ method: 'get', url: getURL, timeout: 5000 })
    const resData = response.data
...
async function getQNBFinansBankEURUSD() {
  try {
    const response = await axios({ method: 'get', url: getURL, timeout: 5000 })
    const resData = response.data
    const resEURBuy = resData[1]['effectiveBuyingExchangeRateField']
...
async function getQNBFinansBankGAU() {
  try {
    const response = await axios({ method: 'get', url: getURL, timeout: 5000 })
    const resData = response.data
...
function getQNBFinansBankForex() {
  return (
    getQNBFinansBankUSD() +
    getQNBFinansBankEUR() +
    getQNBFinansBankGAU() +
...
module.exports = getQNBFinansBankForex;